...
status: Open
estimation: 1h
source: apache-skills session, 2026-09-09 — discovered while running /address-pr on xinzweb/apache-skills
...
- Observed failure: a task's `claimed_by` was stamped
  `Shines-Laptop.local:/Users/xlj/workspace/xinzweb/apache-skills` at
  claim time. Later, on the exact same machine, exact same working
  directory, running `hostname` returned
  `shines-laptop-1.tail472c99.ts.net` instead (Tailscale had been set up
  ...
  `Shines-Laptop.local:...`, found no match, and returned
  `owned:Shines-Laptop.local:...` — a **false "owned by another agent"**
