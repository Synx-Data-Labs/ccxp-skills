...
- **Spec file (hub repo, human-owned)**: `dev/EPICS.md` in `ROADMAP_TARGET_REPO`.
  ...
  Filed on the hub side as its own task in the hub repo (initial content for the
  five current epics).
...
- [ ] Live: run `epic-status.sh render` from a consumer-repo clone against the
      hub repo's `dev/EPICS.md`; all five epics resolve, cross-repo IDs included
...
- Blocked in practice on the consumer box having `ROADMAP_TARGET_REPO` set
  (tracked as its own task in the consumer repo) — the script must degrade gracefully
