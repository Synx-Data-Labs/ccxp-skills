...
|-------------|---------------------|
| Planning game | Pre-IPM design pass (interactive-only, Phase 2a.3, run as `/incept` per candidate) readies candidates during the week; Monday IPM (Phase 2a, cron) budget-cuts whatever's design-ready → ipm-weekly.md; **continuous pre-IPM staging** via `/stage` (candidates accrue all week under `## Candidates`, IPM folds them in) |
