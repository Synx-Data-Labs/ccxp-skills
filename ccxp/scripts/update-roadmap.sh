...
roadmap_commit_pr() {
  ...
  cd "$target" || return 1
  bash ~/.claude/skills/_docs/lint-docs.sh --fix || true   # doc-lint guard — shared script, runs for real here too now (T20260719-111051), see /gcpr Step 1.5 (T20260627-192311)
  git add dev/ROADMAP.md
  git commit -m "docs(roadmap): IPM commit $(date +%Y-%m-%d) — promotions, demotions, last-updated tick"
  local branch
  branch=$(git branch --show-current)
  git push -u origin "$branch"
  bash ~/.claude/skills/_gh/gh.sh pr create --repo "$ROADMAP_TARGET_REPO" \
