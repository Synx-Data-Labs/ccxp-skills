...
- **Progress** — `/drive` did **not** invoke its own Escalation Rules (`drive/SKILL.md`'s Slack-and-stop paths) at any point this cycle, **and** either merged something or left a task in a normal transient wait state (e.g. PR open, CI running, awaiting review) — the no-escalation condition gates the whole bucket, not just the wait-state case, since a multi-task cycle can merge one task via blocker recursion and still escalate on another before returning: reset `stuck_count = 0`, then `ScheduleWakeup(delaySeconds: 60, prompt: "/autopilot until <end_time> stuck=0", noop: false, reason: "continuing autopilot — last /drive cycle made progress")`. Done with this turn.
...
- **Stuck backoff is silent — from `/autopilot`'s side.** `/autopilot` itself only posts to Slack once, at the final Phase 5 stop — a backing-off cycle just reschedules quietly (`noop: true`) and moves on. But a Stuck cycle folded in from `/drive`'s own Escalation Rules may already have sent its own Slack message before `/autopilot` ever classified the outcome; that's `/drive`'s notification, not a repeat autopilot post, and it can recur once per backoff retry for as long as the underlying escalation cause persists.
