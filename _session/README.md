...
PR ownership is **not tracked separately** (T20260622-404636). A PR is owned by whoever owns the **task it implements** — that task's `claimed_by: cc1-<machine-id>:<path-hash>` on `main` (the lock below). There is no PR-resident marker. `task_claim.sh pr-owner <pr>` resolves PR → task → `claimed_by` and answers "may I drive this PR?":

| Output | Meaning |
|--------|---------|
| `mine` | this (machine, clone) agent holds the PR's task — proceed |
...
- **A dead owner's claim self-clears.** Same (machine, clone) agent: its next pickup runs `release-others` (release-on-pickup) and frees the stale claim. Foreign dead agent: the [reclaim sweep](#reclaiming-dead-claims) frees it once the claim is stale (open PRs are reclaimed by **PR-activity** staleness, not main-branch commits — T20260622-404636).
...
**Caveat (load-bearing):** because the agent identity is the (machine, clone) pair, two sessions sharing a clone are indistinguishable. The cron-vs-human anti-steal (the failure mode the old per-session marker guarded) now rests on the **one-session-per-clone** convention — cron, interactive, and `/tmp` cross-repo clones are distinct paths. Keep them distinct.
...
The durable "who owns this task" lock. Unlike the Project-board mirror above, it does NOT live on the board: it lives in the task file's `claimed_by: cc1-<machine-id>:<path-hash>` frontmatter, and the **merge-to-`main` arbitrates it atomically** — two sessions racing for the same task collide on the `claimed_by:` line, and the loser's claim PR fails to merge. The `sync-tasks` workflow projects this on-main state onto the Project board (an opaque TEXT field — format-agnostic), so the board reflects the real claim without any board-side write.

**Claimant identity = `cc1-<machine-id>:<path-hash>`** (e.g. `cc1-a1b2c3d4:9f8e7d6c5b4a3210`), still the (machine, clone-path) pair semantically — **not** a per-invocation session id (`T20260615-169917` retired the old `<sid>@<machine>`). It is stable across CC invocations in the same clone (one clone = one session), so a new `/drive`/ccxp run is the *same* claimant and supersedes its own prior claim instead of stacking. Legacy `<sid>@<machine>` claims still parse and defer (never stolen); they age out via the reclaim sweep / `release`. This same identity now also defines **PR ownership** (a PR is owned by the agent holding its task — T20260622-404636); `session_cc_session_id` survives in `_lib.sh` only as a generic per-host helper, used by neither lock.

**Why it is opaque (T20260911-698434).** The identity used to be the plaintext `<hostname>:<clone-path>`. That broke twice over. `hostname` is not stable on one machine — mDNS, a mesh network's own DNS and `ComputerName` all answer differently, and the answer changes when networking software is installed — so a claim stamped under one name later read as *foreign* and produced false "owned by another agent" verdicts against a task's own live owner. And because `claimed_by` lands on `main`, every claim published a machine name, an OS username and a filesystem path into the repo. The current form fixes both: `<machine-id>` is random and cached in `~/.claude/state/machine-id` (never derived from `hostname`, so drift cannot reach it), and `<path-hash>` is `sha256(secret ‖ clone-path)` salted with a separate local secret — not with `<machine-id>`, which is published inside the claim and would leave the path brute-forceable. `_session/claimant-id.sh` is the single definition; `statusline-command.sh` sources it rather than reimplementing the format, which is how the two silently drifted apart before.

The `cc1-` marker rides *inside* the host field rather than being its own `:`-separated field, so splitting on the first `:` still yields something machine-specific. `_tc_is_own_cross_repo_clone` depends on exactly that: had the format been `cc1:<machine-id>:<path-hash>`, the machine half would read as the literal `cc1` for every claim and its "a foreign machine is never mine" guard would pass for everyone.

Resolving the identity **fails closed**. If `~/.claude/state/` cannot be written, a claim aborts with a message rather than degrading to a hostname (which reintroduces the leak) or minting a per-run secret (which would stop the agent recognising its own prior claim — the very failure this replaced, made permanent).

**`claimed_role: ccxp|interactive`** is written beside `claimed_by` and cleared with it. `attribution.sh` used to recover the clone path from `claimed_by` and test it against `ATTRIBUTION_CCXP_PATHS`; a hash makes that impossible, and a per-machine secret means another machine could not reproduce the hash at standup time anyway. The role is the only thing attribution ever consumed, carries no PII, and stays comparable across machines. Pre-migration history still classifies by path, so every historical retro keeps attributing correctly.
...
A claim whose owning agent died (crash / kill / reboot) is freed two ways. **Same (machine, clone) agent:** its next pickup runs `release-others` and drops the stale claim (instant; the common case — the cron/clone restarts regularly). **Foreign dead agent:** `reclaim_sweep.sh --apply` (a `/ccxp` Phase-0 tick) frees it once `reclaimable` reports the claim stale. `reclaimable` is **git/PR-activity** liveness, not a process probe (verified-dead PID detection isn't feasible on stable signals — T20260622-404636):
